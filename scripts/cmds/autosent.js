// Fix for: process.stderr.clearLine is not a function if (typeof process.stderr.clearLine !== "function") { process.stderr.clearLine = () => {}; process.stderr.cursorTo = () => {}; process.stderr.moveCursor = () => {}; } if (typeof process.stdout.clearLine !== "function") { process.stdout.clearLine = () => {}; process.stdout.cursorTo = () => {}; process.stdout.moveCursor = () => {}; }

const schedule = require("node-schedule"); const moment = require("moment-timezone"); const chalk = require("chalk");

module.exports = { config: { name: "autosent", version: "4.4.1", author: "Your Name", countDown: 5, role: 0, description: { en: "24-hour intelligent group companion with MP4 videos (BD Time)", vi: "Hệ thống tin nhắn thông minh 24 giờ kèm video" }, category: "system", guide: { en: "Automatic system. No command required." } },
...
// Goat-Bot does not expose a reliable allThreadID list, so read active groups from threadsData.
async function getThreadIDs() {
  try {
    const threads = await threadsData.getAll();
    const botID = api.getCurrentUserID();
    return threads
      .filter(thread => thread.isGroup && thread.members?.some(member => member.userID == botID && member.inGroup))
      .map(thread => thread.threadID);
  } catch (error) {
    console.error("[autosent] Could not load group list:", error.message);
    return [];
  }
}
// ========== VIDEO LINKS (Catbox) ==========
...
// ========== MAIN MESSAGES (Every 2 Hours - BD Time) ==========
const messages = {
  "00:00": {
    texts: [
...
async function sendWithVideo(text, videoUrl, threadID) {
  try {
    let attachment = null;
    if (videoUrl && global.utils?.getStreamFromURL) {
...
? Total Messages: ${total} ? Active Members: ${members}

? Activity Level: ${activity}

━━━━━━━━━━━━━━━━━━ ? নতুন দিন শুরু হয়েছে। Let's make today better. ?`;
...
// ====================== MAIN MESSAGES + MP4 (Every 2 Hours - BD Time) ======================
