    ...
    onStart: async function ({ api, event, args, message, role }) {
        ...
        if (subCommand === "help") {
            let helpMsg = "━━━━━ 𝐆𝐃𝐑𝐈𝐕𝐄 𝐇𝐄𝐋𝐏 ━━━━━\n\n" +
                          "🔹 [শুধু লিংক] ➔ ড্রাইভ সেভ ছাড়া ডিরেক্ট প্লে\n" +
                          "🔹 .gdrive save ➔ অটো নাম্বারিংয়ে ফাইল সেভ\n" +
                          "🔹 .gdrive save [নাম] ➔ কাস্টম নামে ফাইল সেভ\n" +
                          "🔹 .gdrive [নাম/নাম্বার] ➔ ফাইল চ্যাটে প্রদর্শন/প্লে\n" +
                          "🔹 .gdrive file [নাম/নাম্বার] ➔ ড্রাইভ ফাইল লিংক\n" +
                          "🔹 .gdrive del [নাম/নাম্বার] ➔ ড্রাইভ থেকে স্থায়ী ডিলিট\n" +
        ...
        if (subCommand === "user") {
            ...
            if (args[1] === "add") {
                ...
                return message.reply(`✅ ইউজার (UID: ${targetUID}) সফলভাবে ড্রাইভ এক্সেস তালিকায় যুক্ত হয়েছে।`);
            }

            if (usersData.allowedUsers.length === 0) return message.reply("⭕ পারমিশন পাওয়া কোনো ইউজারের তালিকা নেই।");
        ...
        if (subCommand === "edit") {
            ...
            if (!oldName || !newName) return message.reply("❌ সঠিক নিয়ম: .gdrive edit [পুরাতন_নাম] [নতুন_নাম]");

            const fileData = getFileData(oldName);
            if (!fileData) return message.reply("❌ এই নামের কোনো ফাইল ডাটাবেসে পাওয়া যায়নি!");
            ...
            try {
                ...
                return message.reply(`✅ ফাইলের নাম '${oldName}' থেকে বদলে সফলভাবে '${newName}' করা হয়েছে!`);
            } catch (err) {
                return message.reply(`❌ নাম পরিবর্তন ব্যর্থ হয়েছে: ${err.message}`);
            }
        }
        ...
        if (subCommand === "del" || subCommand === "delete") {
            ...
            try {
                ...
                fs.writeJsonSync(LINKS_PATH, allLinks);
                return message.reply(`🗑️ '${targetName}' ফাইলটি ড্রাইভ থেকে সফলভাবে ডিলিট করা হয়েছে!`);
            } catch (err) {
                return message.reply(`❌ ডিলিট ব্যর্থ হয়েছে: ${err.message}`);
            }
        }
        ...
        if (subCommand === "file") {
            ...
            const fileData = getFileData(targetName);
            if (!fileData) return message.reply("❌ ফাইলটি পাওয়া যায়নি।");

            // লিংক টেক্সট আগে পাঠানো হচ্ছে
            await message.reply(`📁 𝐅𝐢𝐥𝐞: ${targetName}\n🔗 𝐃𝐨𝐰𝐧𝐥𝐨𝐚𝐝 𝐋𝐢𝐧𝐤:\n${fileData.link}`);

            // এরপর video/audio/image/gif হলে অটোমেটিক প্রিভিউ দেখানো হচ্ছে (শুধু প্লেব্যাক, নতুন করে সেভ হচ্ছে না)
            if (["video", "audio", "image", "gif"].includes(fileData.type)) {
                try {
                    const wait = await message.reply(`⏳ '${targetName}' প্রিভিউ লোড হচ্ছে...`);
                    const res = await drive.files.get({ fileId: fileData.id, alt: "media" }, { responseType: "stream" });
                    await message.unsend(wait.messageID);
                    return message.reply({ body: `✅ Preview: ${targetName}`, attachment: res.data });
                } catch (e) {
                    return message.reply(`⚠️ লিংক পাওয়া গেছে কিন্তু প্রিভিউ লোড করতে সমস্যা হয়েছে: ${e.message}`);
                }
            }
            return;
        }
        ...
        if (subCommand === "save") {
            if (!messageReply || !messageReply.attachments || messageReply.attachments.length === 0) {
                return message.reply("❌ দয়া করে কোনো ছবি/ভিডিওতে রিপ্লাই দিয়ে .gdrive save লিখুন।");
            }
            ...
            try {
                ...
                return message.reply(`❌ সেভ করতে সমস্যা হয়েছে: ${e.message}`);
            }
        }

        if (args[1] === "list") {
            const catType = args[0].toLowerCase();
            const allLinks = fs.readJsonSync(LINKS_PATH);
            const filtered = Object.keys(allLinks).filter(k => allLinks[k].type === catType);
            if (filtered.length === 0) return message.reply(`⭕ ${catType} ক্যাটাগরিতে কোনো ফাইল সেভ করা নেই।`);

            let listMsg = `📂 𝐆𝐃𝐑𝐈𝐕𝐄 ${catType.toUpperCase()} 𝐋𝐈𝐒𝐓:\n\n`;
            for (let i = 0; i < filtered.length; i++) {
                listMsg += `${i + 1}. ${filtered[i]}\n`;
            }
            listMsg += "\n👉 যে ফাইলটি প্লে করতে চান, তার নম্বর টাইপ করে রিপ্লাই দিন।";

            return message.reply(listMsg).then(info => {
                global.GoatBot.onReply.set(info.messageID, {
                    commandName: this.config.name,
                    type: "list_choose",
                    author: senderID,
        ...
        const urls = extractAllUrls(event.body);
        if (urls.length > 0) {
            try {
                const wait = await message.reply("⏳ লিংক থেকে ফাইলটি প্রসেস করা হচ্ছে (No Save)...");
                const resStream = await axios({ method: "get", url: urls[0], responseType: "stream" });
                await message.unsend(wait.messageID);
                return message.reply({ body: "✅ Temporary Playback:", attachment: resStream.data });
            } catch (err) {
                return message.reply(`❌ লিংক প্লে করতে সমস্যা হয়েছে: ${err.message}`);
            }
        }

        const checkName = args.join(" ").trim();
        if (checkName) {
            const fileData = getFileData(checkName);
            if (fileData) {
                try {
                    const wait = await message.reply(`⏳ '${checkName}' ড্রাইভ থেকে লোড হচ্ছে...`);
                    const res = await drive.files.get({ fileId: fileData.id, alt: "media" }, { responseType: "stream" });
                    await message.unsend(wait.messageID);
                    return message.reply({ body: `✅ Playing: ${checkName}`, attachment: res.data });
                } catch (e) {
                    return message.reply(`❌ ফাইলটি প্লে করতে সমস্যা হয়েছে: ${e.message}`);
                }
            } else {
                return message.reply("❌ এই নামে বা নাম্বারে কোনো ফাইল বা কমান্ড পাওয়া যায়নি। সাহায্য পেতে লিখুন: .gdrive help");
            }
        }
    },

    onReply: async function ({ event, message, Reply }) {
        const { author, type, list, messageID } = Reply;
        ...
        global.GoatBot.onReply.delete(messageID);
        const selectedItem = list[choice - 1];

        if (type === "user_remove") {
            const usersData = fs.readJsonSync(USERS_PATH);
            usersData.allowedUsers = usersData.allowedUsers.filter(u => u !== selectedItem);
            fs.writeJsonSync(USERS_PATH, usersData);
            return message.reply(`🗑️ ইউজার (UID: ${selectedItem}) সফলভাবে অনুমতি তালিকা থেকে রিমুভ হয়েছে।`);
        }

        if (type === "list_choose") {
            const allLinks = fs.readJsonSync(LINKS_PATH);
            const fileData = allLinks[selectedItem.toLowerCase()];
            try {
                const wait = await message.reply(`⏳ '${selectedItem}' ড্রাইভ থেকে লোড হচ্ছে...`);
                const localDrive = drive;
                const res = await localDrive.files.get({ fileId: fileData.id, alt: "media" }, { responseType: "stream" });
                await message.unsend(wait.messageID);
                return message.reply({ body: `✅ Playing selected: ${selectedItem}`, attachment: res.data });
            } catch (e) {
                return message.reply(`❌ ফাইলটি আনতে সমস্যা হয়েছে: ${e.message}`);
            }
        }
    }
};
