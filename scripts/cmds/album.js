...
module.exports = {
  config: {
    name: "album",
    version: "1.0.0",
    role: 0, // Goat-Bot-V2 uses "role" instead of "hasPermssion" (0 = everyone, 1 = group admin, 2 = bot admin)
    author: "Dipto Modified By SAHU",
    countDown: 5, // Goat-Bot-V2 uses "countDown" instead of "cooldowns"
    description: "Displays album options for selection.",
    category: "media", // Goat-Bot-V2 categories are conventionally lowercase
    guide: {
      en:
        "{pn} to see the list\n" +
        "{pn} add [cartoon/photo/lofi/sad/islamic/funny/horny/anime/aesthetic/cat/lyrics/love/sigma] (reply to a media)",
    },
  },
  ...
  onStart: async function ({ api, event, args, message }) {
    if (!args[0]) {
      ...
      // message.reply returns the sent message's info (contains messageID)
      const info = await message.reply(msg);

      // Goat-Bot-V2 registers reply-listeners on global.GoatBot.onReply (a Map),
      // keyed by the sent message's messageID
    ...
    // ------------ Video Add via URL ------------
    const d1 = args[1] ? args[1].toLowerCase() : (args[0] ? args[0].toLowerCase() : "");
    const validCommands = [
    ...
    try {
      ...
      console.error(error);
      message.reply(`Failed to upload media.\nError: ${error.message || error}`);
    }
  },

  onReply: async function ({ api, event, Reply, message }) {
    // Only the original requester's reply should be processed
    ...
    if (isNaN(reply) || reply < 1 || reply > 20) {
      return message.reply("Please reply with a number between 1 and 20");
    }
    ...
    if ((reply === 7 || reply === 15) && event.senderID !== admin) {
      return message.reply("Only admin can use it!");
    }
    ...
    try {
      ...
      // done handling this reply chain, remove it from the onReply map
      global.GoatBot.onReply.delete(Reply.messageID);
    } catch (error) {
      console.error(error);
      message.reply(`An error occurred while fetching the media.\n${error.message || error}`);
    }
  },
};
